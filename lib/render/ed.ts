 ...
 *  Разметка лежит рядом файлом (`ed-skeleton.ts` знает путь, он только серверный),
 *  чтобы её можно было открыть в браузере и сличить с документом, скопированным из
 *  редактора navi. Здесь — только каталог и правила: этот файл тянут и клиентские
 *  компоненты, поэтому ничего из node в нём быть не должно.
 *
 *  Идентификатор шаблона — `ed`. */
export const ED_TEMPLATE_ID = "ed";
