...
export default function EmptyState({ kind }: { kind: "sources" | "settings" }) {
  ...
  const map = {
    ...
    settings: { icon: "gear", h: lang === "ru" ? "Настройки" : "Settings", s: lang === "ru" ? "Профиль, команда, интеграции и модель извлечения." : "Profile, team, integrations and extraction model." },
  }[kind];
  return (
    <div className="col" style={{ alignItems: "center", justifyContent: "center", height: "100%", color: "var(--text-3)", gap: 16 }}>
      <span style={{ width: 56, height: 56, borderRadius: 14, display: "grid", placeItems: "center", border: "1px solid var(--line-2)", background: "var(--surface-1)" }}><Icon name={map.icon} size={24} /></span>
